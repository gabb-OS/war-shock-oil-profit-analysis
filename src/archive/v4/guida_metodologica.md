...
### Versione aggiornata post-run 28 aprile 2026
...
### 2.10 ~~Ordinamento pipeline: 3_05 girava dopo 3_02/3_03~~ [RISOLTO in v3]

**Problema originale:** `3_03_did.py` era progettato per leggere `data/3_cp.csv` (τ MCMC)
e usarlo come cutoff pre/post, ma il file veniva prodotto da `3_05_changepoint.py` che
girava *dopo* di lui. Analogamente, `3_02_tests.py` usava date geopolitiche hardcoded
ignorando completamente i τ empirici disponibili.

**Conseguenza:** tutte le finestre pre/post nei test A, B e C erano definite dalla data
geopolitica fissa anziché dal τ rilevato dai dati. `3_03` usava silenziosamente il
fallback hardcoded senza errori visibili, rendendo il bug difficile da individuare.

**Fix v3:** `3_run.py` riordinato: `3_01 → 3_05 → 3_02 → 3_03 → 3_04`. `3_02_tests.py`
aggiornato per leggere `3_cp.csv` e usare τ per-carburante (`Crack_Benz`, `Crack_Dies`)
come cutoff per i test di Famiglia B, e τ medio per Famiglia A. L'ordine è ora
scientifico: prima rilevi *quando* il mercato si rompe, poi testi *cosa* è cambiato.

### 2.11 Svezia diesel — paese controllo non valido per il DiD [NUOVO — CRITICO]

**Problema:** la Svezia aveva la **Reduktionsplikt**, un obbligo di miscelazione di
biocarburante nel diesel con il mandato più alto in UE: 30,5% nel periodo 2022–2023
(alzato da 26% a fine 2021). Il prezzo pompa SE diesel include quindi il costo del
biocarburante, mentre il wholesale di riferimento (Gas Oil ICE) è fossile puro.

**Meccanismo di distorsione:**
```
margine_SE_diesel = pompa_SE − gasoil_ICE
                  = (fossile + biofuel_30.5% + tasse) − gasoil_ICE
                  = margine_reale + COMPONENTE_BIOFUEL_GONFIATA
```

Il "margine" svedese osservato è artificialmente elevato di ~0,10–0,15 EUR/L per il
periodo 2022–2023 rispetto ai paesi con mandati più bassi (DE: mandato volume standard,
IT: nessun mandato di riduzione comparabile).

**Il crollo a gennaio 2024:** il parlamento svedese ha tagliato il mandato dal 30,5%
al 6% a partire da gennaio 2024 → crollo del prezzo diesel SE di ~0,35 EUR/L (≈4 SEK).
Questo appare nei dati come uno shift brusco verso il basso nel diesel SE, portandolo
ad allinearsi con gli altri paesi. Non è un evento geopolitico: è pure policy.

**Conseguenza per il DiD:**
- Finestra Ucraina (2022–2022): margine SE diesel gonfiato → δ_DiD IT-SE diesel distorto verso il basso (IT sembra migliore di quanto sia)
- Finestra 2024+: SE diesel crolla artificialmente → qualsiasi finestra che attraversa gennaio 2024 è inquinata dalla rottura strutturale svedese
- PTA già violata per SE diesel Ucraina (p=0.001) — probabilmente causata proprio dalla Reduktionsplikt

**Fix necessario in `3_01_data.py` e `3_03_did.py`:**
```python
# Commento attuale (SBAGLIATO):
# SE - Svezia : nessun intervento nel periodo analizzato

# Commento corretto:
# SE diesel: ESCLUSA — Reduktionsplikt 30.5% (2022-23) gonfia margine osservato
#            di ~0.10-0.15 EUR/L; crollo strutturale a gen 2024 (30.5%→6%).
#            Non è un paese controllo valido per il DiD diesel.
# SE benzina: usabile con cautela (mandato 7.8%, effetto < 2 ct/L).
```

**Impatto sui risultati esistenti:** i δ_DiD IT-SE diesel vanno considerati non
interpretabili. I δ_DiD IT-DE diesel (PTA valida, nessun problema strutturale comparabile
per la DE) rimangono i riferimenti affidabili.
...
## 12. Roadmap Correttiva — Priorità Aggiornate v3

| Priorità | Issue | Stato | Fix | Script |
|---|---|---|---|---|
| 1 | SE diesel Reduktionsplikt: paese controllo non valido (§2.11) | **APERTO — CRITICO** | Escludere SE dal DiD diesel; aggiornare commento in `3_01` e logica in `3_03` | 3_01, 3_03 |
| 2 | Script 05 carica DiD v1 (§2.6) | **APERTO — CRITICO** | Cambiare path da `auxiliary_pvalues.csv` a `did_results_v2.csv` | 05 |
| 3 | PTA violata in 5/8 DiD (§2.7) | **APERTO** | DiD con trend lineari (event study); alcune violazioni probabilmente causate da Reduktionsplikt SE (§2.11) | 3_03 |
| 4 | δ_DiD anomali v2 vs v1 (§2.8) | **DA INVESTIGARE** | Verificare finestra/definizione in script 3_03 | 3_03 |
| 5 | MCMC Brent Ucraina Rhat=1.162 (§2.9) | **APERTO — PERSISTENTE** | Re-run con tune=10000, prior ristretto | 3_05 |
| 6 | HAC BW≥n/2 per Ucraina/Iran-Is. benzina (§2.4) | **PARZIALMENTE GESTITO** | Segnalato in output; considerare subsample bootstrap | 3_02 |
| 7 | Naming mismatch file (run checker) | **APERTO — MINORE** | Aggiornare 3 path nel checker | 3_run |
| 8 | Convergenza Hormuz (n_post=12, n_eff<5) | **STRUTTURALE** | Attendere n_post≥20 prima di re-run confermativo | 3_02, 3_03 |
| ~~—~~ | ~~Ordinamento pipeline (3_05 dopo 3_02/3_03)~~ | ✓ **RISOLTO in v3** | 3_run riordinato; 3_02 aggiornato per leggere 3_cp.csv | — |
